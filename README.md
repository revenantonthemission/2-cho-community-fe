...
| `/detail?id=` | `post_detail.html` | 게시글 상세 — 마크다운 렌더링, 좋아요/북마크/공유, 댓글 트리, 투표, 연관 게시글 |
...
| `/signup` | `user_signup.html` | 회원가입 — 실시간 유효성 검사, 프로필 이미지 |
...
| `/user-profile?id=` | `user-profile.html` | 사용자 프로필 — 작성 글, 팔로우/차단/DM, 관리자 정지 |
...
`ThemeService`가 `[data-theme="dark"]` 속성을 토글하면, `variables.css`의 다크 테마 토큰이 활성화되어 22개 CSS 파일이 자동 전환됩니다. FOUC(Flash of Unstyled Content) 방지를 위해 17개 HTML에 인라인 스크립트로 초기 테마를 즉시 적용합니다.
...
17개 HTML을 개별 엔트리포인트로 등록하는 MPA(Multi-Page Application) 구성입니다.
