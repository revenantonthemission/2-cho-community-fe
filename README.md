...
- 게시글 CRUD, 댓글(대댓글), 좋아요, 북마크, 검색/정렬(5종), 카테고리, 태그
- 마크다운 에디터 (GFM, 코드 구문 강조, 이미지 DnD, 미리보기)
- 실시간 알림 (WebSocket + 폴링 폴백) · DM 쪽지 · @멘션 하이라이트
- 팔로우/팔로잉 피드 · 투표(Poll) · 연관 게시글 추천 · 읽은 글 표시
...
┌─────────────────────────────────────────────────────┐
│  app/*.js (진입점)                                    │
│  HTML 페이지와 1:1 매핑, Controller 초기화             │
└──────────────┬──────────────────────────────────────┘
               │
       ┌───────▼───────┐
       │  Controller   │ 비즈니스 로직, 상태 관리
       │  (정적 메서드)  │ Model과 View 조율
       └───┬───────┬───┘
           │       │
     ┌─────▼──┐ ┌──▼─────┐
     │ Model  │ │  View  │
...
│   │   ├── mention.js         # @멘션 TreeWalker 하이라이트
...
└── css/
    ...
    ├── variables.css           # 100+ 디자인 토큰 (3단계 체계)
    ├── base.css               # 리셋, 타이포그래피
    ├── layout.css             # 헤더, 컨테이너
...
| 클린 URL | HTML 파일 | 설명 |
| -------- | --------- | ---- |
| `/main` | `html/post_list.html` | 메인 피드 — 카테고리 탭, 검색, 정렬(5종), 팔로잉 필터, 무한 스크롤 |
| `/detail?id=` | `html/post_detail.html` | 게시글 상세 — 마크다운 렌더링, 좋아요/북마크/공유, 댓글 트리, 투표, 연관 게시글 |
| `/write` | `html/post_write.html` | 게시글 작성 — 마크다운 에디터(14버튼), 카테고리, 태그, 다중 이미지, 투표, 임시 저장 |
| `/edit?id=` | `html/post_edit.html` | 게시글 수정 — 기존 데이터 프리필, 이미지 관리 |
| `/login` | `html/user_login.html` | 로그인 — 정지 사용자 안내, 이메일 인증 유도 |
| `/signup` | `html/user_signup.html` | 회원가입 — 실시간 유효성 검사, 프로필 이미지 |
| `/find-account` | `html/user_find_account.html` | 계정 찾기 — 탭 UI (이메일 찾기 / 비밀번호 재설정) |
| `/password` | `html/user_password.html` | 비밀번호 변경 |
| `/edit-profile` | `html/user_edit.html` | 프로필 수정 — 닉네임, 프로필 이미지 |
| `/user-profile?id=` | `html/user-profile.html` | 사용자 프로필 — 작성 글, 팔로우/차단/DM, 관리자 정지 |
| `/notifications` | `html/notifications.html` | 알림 — 좋아요/댓글/멘션/팔로우, 읽음/삭제, 무한 스크롤 |
| `/my-activity` | `html/my-activity.html` | 내 활동 — 탭 UI (글/댓글/좋아요/북마크/차단) |
| `/verify-email` | `html/verify-email.html` | 이메일 인증 결과 표시 |
| `/messages` | `html/dm_list.html` | DM 대화 목록 — 최근 메시지 미리보기, 읽지 않은 수 |
| `/messages/inbox` | `html/dm.html` | DM 데스크톱 통합 — 좌우 분할 레이아웃 (768px 이상 자동 전환) |
| `/messages/detail?id=` | `html/dm_detail.html` | DM 상세 — 실시간 메시지 수신, 마크다운 컴팩트 에디터 |
| `/admin/reports` | `html/admin_reports.html` | 관리자 신고 관리 — 상태 필터, 처리/기각, 정지 연동 |
| `/admin/dashboard` | `html/admin_dashboard.html` | 관리자 대시보드 — 통계 카드, 일별 추이, 사용자 관리 |
...
- `ws:message_deleted` — WebSocket DM 메시지 삭제 → 삭제 플레이스홀더 표시
- `ws:message_read` — WebSocket DM 읽음 확인 → 체크마크 업데이트
- `ws:typing` — WebSocket 타이핑 인디케이터 → 입력 중 표시
...
`css/variables.css`에 100개 이상의 CSS Custom Properties를 3단계 체계(Primitive → Semantic → Component)로 정의하고, 22개 CSS 파일에서 `var()` 참조로 일관성을 보장합니다.
...
| Semantic (상태) | `--state-hover-bg`, `--state-active-bg`, `--state-disabled-opacity` |
| Component (버튼) | `--btn-primary-bg`, `--btn-primary-hover`, `--btn-disabled-bg` |
| Component (카드) | `--card-border`, `--card-hover-shadow`, `--card-badge-radius` |
| Component (입력) | `--input-border`, `--input-focus-ring`, `--input-placeholder` |
